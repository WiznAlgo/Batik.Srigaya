...
import { ArrowUpRight, Instagram, Link2, Mail, MapPin, PhoneCall } from "lucide-react";
...
const contactCards = [
  {
    ...
    note: "Lihat karya dan update terbaru",
    icon: Instagram,
    external: true,
  },
  {
    label: "Linktree",
    value: "Semua tautan resmi",
    href: contacts.linktree,
    note: "Buka semua akses resmi",
    icon: Link2,
    external: true,
  },
  {
    label: "WhatsApp",
    value: "+62 812-2583-3053",
    href: contacts.whatsapp,
    note: "Chat langsung untuk pesanan",
    icon: PhoneCall,
    external: true,
  },
  {
    label: "Email",
    value: "batiksrigaya@gmail.com",
    href: contacts.email,
    note: "Kirim pesan via Gmail",
    icon: Mail,
    external: false,
  },
];
  ...
  return (
    <section className="px-4 py-8 md:px-6 md:py-16">
      <div className="mx-auto grid w-full max-w-[1280px] gap-8 lg:grid-cols-[1.06fr_0.94fr]">
        <div
          data-reveal
          className="rounded-[34px] border border-[rgba(224,194,127,0.18)] bg-[linear-gradient(180deg,rgba(255,255,255,0.07),rgba(255,255,255,0.03))] p-7 opacity-0 translate-y-6 shadow-[0_18px_60px_rgba(0,0,0,0.3)] transition duration-700 md:p-9"
        >
          <SectionHeading
        ...
        <div data-reveal className="grid gap-4 opacity-0 translate-y-6 transition duration-700">
          <div className="rounded-[34px] border border-white/10 bg-white/6 p-7 shadow-[0_18px_60px_rgba(0,0,0,0.3)]">
            ...
            <div className="mt-6 grid gap-3">
              {contactCards.map((item) => {
                ...
                return (
                  <a
                    ...
                    className="group flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 transition duration-300 hover:-translate-y-0.5 hover:border-[rgba(224,194,127,0.25)] hover:bg-white/8"
                  >
                    <div className="flex items-start gap-3">
                      <span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-black/20 text-[var(--gold-2)]">
                    ...
                    <ArrowUpRight className="h-4 w-4 shrink-0 text-white/40 transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-[var(--gold-2)]" />
                  </a>
                );
              })}
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="mb-2 flex items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black/20 text-[var(--gold-2)]">
                    <MapPin className="h-4 w-4" />
                  </span>
                  <span className="text-[11px] uppercase tracking-[0.28em] text-[var(--gold-2)]">
                    Alamat Butik
                  </span>
                </div>
                <p className="text-sm leading-7 text-white/78">{contacts.address}</p>
              </div>
            </div>
          </div>
